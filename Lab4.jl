...
function Gauss(std, mean, x)
    ...
    #TODO 
	p = 1 / (std * sqrt(2π)) * exp(-((x - mean) / std)^2 / 2)
	return p
end
...
function maxHypo(self::NBGaussian)
    ...
    #TODO
    maxProbability = first(keys(self.hist))
    for hypo in keys(self.hist)
        if self.hist[hypo] > self.hist[maxProbability]
            maxProbability = hypo
        end
    end  
...
function update(self::NBGaussian, data)        
    ...
    for hypo in keys(self.hist)
        #self.hist[hypo]=?
		self.hist[hypo] *= likelihood(data, hypo)	
	end
	#Normalization        
    #TODO: s=P(data)
    #s=?    
